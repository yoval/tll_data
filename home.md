...
2. 中台单次最多导出30万条数据。
3. **导出**的数据中最后一个单元格`=CODE(RIGHT(A2,1))`是9，即制表符。可使用“替换”转换为常规格式。
...
中台中，日期格式是`YYYYMMDD`格式，

`SELECT CURDATE() as today;` 返回的是`2024-05-23` ，

`SELECT DATE_SUB(CURDATE(), INTERVAL 1 DAY) AS yesterday;` 返回的是`2024-05-22` 。

转为相同格式可用`SELECT REPLACE(DATE(CURDATE()), '-', '') AS today;`，返回的是`20240523`。
